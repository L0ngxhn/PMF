        ...
        while self.epoch < self.maxepoch:  # 检查迭代次数
            ...
            for batch in range(self.num_batches):  # 每次迭代要使用的数据量
                ...
                if batch == self.num_batches - 1:
                    ...
                    self.mse_train.append((obj / pairs_train))
                ...
                if batch == self.num_batches - 1:
                    ...
                    self.mse_test.append(np.square(np.linalg.norm(rawErr)) / (pairs_test))

                    # Print info
                    if batch == self.num_batches - 1:
                        print('Training RMSE: %f, Test RMSE %f' % (self.rmse_train[-1], self.rmse_test[-1]))

                        if(self.mse_test[-1] < self.best_mse_test):
                            self.best_mse_test = self.mse_test[-1]
                        print('Training MSE: %f, Test MSE %f' % (self.mse_train[-1], self.mse_test[-1]))

        print('Best Test MSE : %f' % (self.best_mse_test))
